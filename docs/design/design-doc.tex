...
inode blocks.
%As the filesystem has fixed number of inode blocks, which also controls the
%number of files supported by the system.
...
Kawkab has fixed size data blocks. Each data block has a unique ID, generated as a UUID.
Each block is persistently stored as a file in the system. The name of the
file is based on the UUID: the UUID is converted to an ASCII name using base64 encoding.
The ASCII name is then divided into a file path -- each two characters become a directory
name in the ASCII representation of the UUID. For example, a UUID \texttt{ABCDEFGH} is
stored as a file \texttt{/AB/CD/EF/HI/ABCDEFGH}.
