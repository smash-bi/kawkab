...
public final class PrimaryNodeService extends PrimaryNodeImplBase {
	...
	@Override
	public void segmentExists(KSegmentRequest req, StreamObserver<KExistsResponse> responseObserver) {
		BlockID bid = new DataSegmentID(req.getInumber(), req.getBlockInFile(), req.getSegmentInBlock(), req.getRecordSize());
	...
	@Override
	public void getSegment(KSegmentRequest req, StreamObserver<KSegmentResponse> responseObserver) {
		DataSegmentID bid = new DataSegmentID(req.getInumber(), req.getBlockInFile(), req.getSegmentInBlock(), req.getRecordSize());
		...
		try {
			blockBytes = getSegmentData(bid, req.getOffset());
			if (blockBytes == null)
	...
	private ByteString getSegmentData(DataSegmentID bid, final int offset) throws IOException, KawkabException {
		DataSegment block = null;
		ByteString blockBytes = null;
		try {
			block = (DataSegment)cache.acquireBlock(bid);
			block.loadBlock();
			blockBytes = block.byteString(offset);
	...
	public void getInodesBlock(KInodesBlockID kid, StreamObserver<KInodesBlockResponse> responseObserver) {
		InodesBlockID bid = new InodesBlockID(kid.getBlockIndex());
		KErrorCode ec = KErrorCode.SUCCESS;
		...
		ByteString blockBytes = null;
		try {
			blockBytes = getInodesBlockData(bid);
			if (blockBytes == null)
				ec = KErrorCode.BLOCK_NOT_EXIST;
		} catch (IOException | KawkabException e) {
			e.printStackTrace();
	...
	private ByteString getInodesBlockData(InodesBlockID bid) throws IOException, KawkabException {
		Block block = null;
		ByteString blockBytes = null;
		try {
