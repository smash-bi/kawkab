...
def prepare_folders(conf):
    ...
    make_dirs(clients_dir, "clients", conf)
    conf['clients_dir'] = clients_dir

    clients_dir = conf['run_dir']+'/servers'
    make_dirs(clients_dir, "servers", conf)
...
def start_clients(conf):
    ...
    opts = ' mport=%d wt=%d nc=%d bs=%d rs=%d nf=%d fp=%s typ=%s tc=%d td=%d wr=%d iat=%d wmup=%d '%(
...
def get_results(conf):
    p = conf['test_params']
    cmd = "echo %d, %d, %d, %d, %d, %s, %d, %d, $(tail -n 1 %s/write-results.csv) >> %s/all-results.csv"%(
        p['tc'], p['nc'], p['fpc'], p['rs'], p['bs'], p['test_prefix'], p['iat'], p['test_run'],
        conf['test_dir'], conf['exp_dir'])
    run_cmd(cmd)

    print '============================================'
    cmd = "cat %s/write-results.json"%(conf['test_dir'])
    run_cmd(cmd)
    print '============================================'
    cmd = "cat %s/write-results.csv"%(conf['test_dir'])
    run_cmd(cmd)
    print '============================================'
...
def run_batch(conf):
    ...
    run_t *= len(conf['record_size'])*len(conf['write_ratio'])*len(conf['iat'])*len(conf['test_runs'])
                        ...
                        for record_size in conf['record_size']:
                            for iat in conf['iat']:
                                for test_run in conf['test_runs']:
                                    ...
                                    conf['test_id'] = '%s-%s-nc%d-bs%d-rs%d-nf%d-wr%d-iat%d'%(
                                                                                            test_type,
                                                                                            test_prefix,
                                                                                            numClients,
                                                                                            batch_size,
